(function (global) {
  ...
  const SITUATIONS = {
    money: {
      ...
      side:{label:"부업·새 수입을 만들고 싶어",object:"새 수입원",cue:"새 아이디어를 실제 돈으로 시험해야 하는 순간",move:"작은 제안 하나에 가격을 붙여 실제 반응 보기",metric:"7일 안에 가격을 붙인 제안 하나를 실제로 보여주고 반응 확인하기"},
      flow:{label:"앞으로 돈 흐름이 궁금해",object:"앞으로의 돈 흐름",cue:"좋은 시기를 기다리면서 무엇을 할지 정해야 하는 순간",move:"좋은 구간에 할 돈 행동 하나를 미리 정하기",metric:"다음 기회가 왔을 때 바로 할 돈 행동 하나를 미리 정해두기"},
    },
    career: {
    ...
    love: {
      ...
      relationship:{label:"지금 연애 중이야",object:"현재 연애",cue:"연락·표현·약속에서 작은 서운함이 생기는 순간",move:"서운한 장면 하나를 감정이 커지기 전에 짧게 말하기",metric:"서운함 하나를 너무 오래 묵히기 전에 한 문장으로 말하기"},
    ...
    path: {
      ...
      strength:{label:"내 적성·강점을 알고 싶어",object:"적성과 강점",cue:"내가 뭘 잘하는지 한마디로 정리하고 싶은 순간",move:"잘하는 방식이 반복해서 나타난 장면 세 개를 모으기",metric:"강점이 드러난 장면 3개에서 공통 행동 하나 뽑기"},
    },
    people: {
      friend:{label:"친구·지인 때문에 힘들어",object:"친구·지인 관계",cue:"친구·지인 사이에서 불편함을 말할지 참을지 고민되는 순간",move:"작은 경계 하나를 말하고 상대 반응 보기",metric:"작은 경계 하나를 말하고 상대 반응을 그대로 기록하기"},
      work:{label:"직장 사람 때문에 힘들어",object:"직장 사람과의 관계",cue:"일 때문에 계속 봐야 하는 사람에게 선을 세워야 하는 순간",move:"감정 대신 역할·업무·연락 기준을 문장으로 정하기",metric:"업무 범위·기한·연락 기준 중 하나를 문장으로 고정하기"},
      family:{label:"가족과 자꾸 부딪혀",object:"가족과의 반복 갈등",cue:"가족과 같은 주제로 부딪히고 감정이 오래 남는 순간",move:"상대를 바꾸려 하기보다 서로 부딪히는 주제에서 내가 지킬 범위를 정하기",metric:"부딪히는 주제 하나에서 할 수 있는 범위와 어려운 범위를 한 문장씩 정하기"},
    ...
    mental: {
      burnout:{label:"번아웃이 온 것 같아",object:"번아웃",cue:"해야 할 건 많은데 몸과 마음이 더는 따라오지 않는 순간",move:"해야 할 일을 늘리기 전에 부하 하나를 실제로 덜어내기",metric:"7일 동안 해야 할 일 하나를 실제로 빼고 수면·피로 변화를 기록하기"},
      ...
      recover:{label:"다시 컨디션을 찾고 싶어",object:"컨디션 회복",cue:"조금 나아졌지만 어느 정도까지 다시 움직여도 될지 조심스러운 순간",move:"괜찮았던 수면·식사·움직임 하나를 반복 일정으로 만들기",metric:"좋은 날에도 활동량을 한꺼번에 크게 늘리지 않기"},
    },
  };
