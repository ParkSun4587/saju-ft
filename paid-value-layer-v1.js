  ...
  const SITUATION_PROFILES = {
    money: {
      ...
      side: {
        ...
        second: "돈을 낸 반응이 있었던 것만 남겨 반복해서 돈이 되는 방식으로 만들어",
      },
      flow: {
        label: "앞으로 돈 흐름이 궁금해",
    ...
    career: {
      ...
      current: {
        ...
        focus: "새 직장을 찾기보다 지금 한 일이 평가·역할·보상으로 실제 연결되게 만드는 게 먼저야",
    ...
    mental: {
      ...
      low: {
        ...
        place: "성과를 재촉하지 않고 작은 행동을 반복할 수 있는 환경",
  ...
  function applySituationNotes(notes, data, mode) {
    ...
    out[3] = {
      ...
      title: isT
        ...
        : "언니랑 7일만 이렇게 바꿔보자",
      desc: isT
        ? `이건 그냥 할 일 세 개 던지는 파트가 아니야. <b>${p.trigger}</b> 때 시작해서 <b>${p.reaction}</b>으로 넘어가는 그 순서를 실제 생활에서 끊는 처방이야.<br><br><b>오늘 먼저</b><br>${p.steps[0]}.<br><br><b>이번 7일 안에</b><br>${p.steps[1]}.<br><br><b>이번 주에 멈출 것</b><br>${p.steps[2]}.<br><br><b>이 처방이 먹히는 신호</b><br>${p.cost} 같은 결과가 예전보다 덜 남고, 같은 장면이 와도 한 번 멈춰서 선택할 여지가 생기는 거야.<br><br>${personal} 완벽하게 지키는 게 목표가 아니라, 예전 반응 대신 다른 선택을 한 번이라도 만드는 게 목표야.`
        : `언니가 그냥 “이거 해” 하고 숙제만 주려는 건 아니야. 보통 <b>${p.trigger}</b> 때 시작해서 <b>${p.reaction}</b>으로 넘어가잖아. 이번 7일은 그 익숙한 순서를 조금 다르게 만들어보는 시간이야.<br><br><b>오늘 먼저</b><br>${p.steps[0]}.<br><br><b>이번 7일 안에</b><br>${p.steps[1]}.<br><br><b>이번 주엔 이것만 멈춰보자</b><br>${p.steps[2]}.<br><br><b>잘 되고 있다는 신호</b><br>${p.cost} 같은 끝맺음이 조금 덜 남고, “또 이렇게 됐네” 하기 전에 네가 한 번 선택할 수 있으면 돼.<br><br>${personal} 다 잘하려고 하지 않아도 돼. 언니는 네가 덜 소모되는 변화 하나만 생겨도 충분하다고 봐.`,
      checklist: isT
        ? "7일 뒤 세 가지 중 뭐가 실제로 먹혔는지 하나만 남겨. 안 먹힌 건 버려도 돼."
        : "7일 뒤 언니한테 보여준다고 생각하고, ‘이건 좀 덜 힘들었어’ 싶은 변화 하나만 남겨보자.",
    };
    ...
    out[4] = {
      ...
      title: isT
        ...
        : "네가 편해지는 사람은 따로 있어",
      desc: isT
        ? `사람을 좋은 사람/나쁜 사람으로 자를 필요는 없어. <b>그 사람 옆에서 네 판단력·체력·기준이 어떻게 변하는지</b> 보면 돼.<br><br><b>가까이 둘 쪽</b><br>${p.keep}.<br><br><b>거리를 조절할 쪽</b><br>${p.cut}.<br><br><b>네가 잘 풀리는 환경</b><br>${p.place}.<br><br><b>판별할 때 세 가지만 봐</b><br>1. 불편한 걸 말했을 때 듣는지.<br>2. 말과 행동이 계속 맞는지.<br>3. 만나고 난 뒤 네 에너지가 회복되는지, 더 소모되는지.<br><br>${personal} 특히 네가 자꾸 설명하고 이해시키느라 힘을 많이 써야 하는 관계라면, 친한 기간보다 지금의 반응을 기준으로 보는 게 맞아.`
        : `언니가 “누굴 끊어”라고 말하려는 건 아니야. 오히려 <b>네가 덜 애써도 안전한 관계인지</b>를 같이 보자는 거야.<br><br><b>곁에 두면 좋은 쪽</b><br>${p.keep}.<br><br><b>조금 거리를 둬도 되는 쪽</b><br>${p.cut}.<br><br><b>네가 숨 쉬기 편한 환경</b><br>${p.place}.<br><br><b>사람을 볼 때 이 세 가지만 기억해</b><br>1. 네가 불편하다고 말해도 관계가 안전한지.<br>2. 말보다 행동이 계속 맞는지.<br>3. 만나고 돌아왔을 때 마음이 편한지, 이유 없이 지쳐 있는지.<br><br>${personal} 오래 봤다는 이유만으로 네가 계속 더 이해하고 더 참아야 하는 관계를 정답처럼 붙잡지는 않았으면 좋겠어.`,
      checklist: isT
        ? "지금 떠오르는 한 사람에게 3개 기준을 대입해. 두 개 이상 계속 어긋나면 거리 조절부터 해."
        : "지금 떠오르는 한 사람을 생각해보고, ‘내가 이 사람 앞에서 나답게 편한가?’부터 한 번 물어보자.",
    };
    return out;
  }
