(function (global) {
  ...
  function fullSajuSections(data, mode) {
    ...
    const sections = [
      ...
      {
        title:"10 · 가까운 18개월",
        body:`${isT ? "기본 상담에서는 이번 질문에 필요한 가까운 시기만 골랐다면, 전체판에서는 <b>삶 전체에 영향을 주는 가까운 구간</b>을 본다." : "기본 상담에서는 이번 질문에 필요한 가까운 시기만 봤지. 여기서는 <b>삶 전체에 영향을 주는 가까운 구간</b>까지 같이 볼게."}<br><br>${nearTimelineCopy()}<br><br>${use("같은 달이어도 돈·일·관계 중 실제 반응이 먼저 오는 영역부터 작게 움직이고, 나머지는 그 결과를 본 뒤 따라가.")}`,
      ...
      {
        title:"11 · 앞으로 5년",
        body:`${isT ? "기본 상담에서 다 공개하지 않은 연도별 흐름이야. 해마다 방향·이유·할 일까지 본다." : "기본 상담에서는 다 보여주지 않았던 연도별 흐름도 여기서는 이어서 볼게. 해마다 좋다/나쁘다로 끝내지 않고 이유와 할 일까지 같이 볼게."}<br><br>${yearTimelineCopy()}<br><br>${use("좋은 해에는 준비한 걸 실제로 확정하고, 조심할 해에는 범위를 줄이는 식으로 5년을 나눠 써.")}`,
