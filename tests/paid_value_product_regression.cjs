...
(async () => {
  ...
  await page.waitForFunction(() =>
    ...
    globalThis.__CONCERN_NOTE_ENGINE_V2__?.version === '5.2.0' &&
  ...
  assert(qa.noteVersion.version === '5.2.0', 'NOTE v3 engine missing');
  ...
  for (const row of qa.situationRows) {
    ...
    assert(row.noteAudit?.version === '5.2.0' && row.noteAudit?.engine === 'classical-causal-full-evidence' && row.noteAudit?.structureFingerprint && row.noteAudit?.synthesisFingerprint,
  ...
  assert(html.includes('./concern-note-engine-v2.js?v=5.2.0'), 'five-answer NOTE script include missing');
