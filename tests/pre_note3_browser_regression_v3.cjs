...
(async () => {
  ...
  // Core runtime checks in browser, including the exact user case.
  {
    ...
    const unexpectedErrs = errs.filter(x => !isExpectedBoundaryDiagnostic(x));
    assert(unexpectedErrs.length === 0, `core browser errors: ${unexpectedErrs.join(' | ')}`);
  ...
  for (const c of CASES) {
    ...
    const unexpectedErrs = errs.filter(x => !isExpectedBoundaryDiagnostic(x));
    assert(unexpectedErrs.length === 0, `${c.id}: browser errors: ${unexpectedErrs.join(' | ')}`);
