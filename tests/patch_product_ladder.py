...
if new_meta not in ps:
    if old_meta not in ps and 'wrapped.__integratedBase = base;' not in ps: raise SystemExit('paid wrapper metadata anchor not found')
...
if new_enriched not in ts:
    if old_enriched not in ts and new_enriched.strip() not in ts: raise SystemExit('integrated enriched assertion anchor not found')
    if old_enriched in ts: ts = ts.replace(old_enriched, new_enriched, 1)
it.write_text(ts, encoding='utf-8')

# ---- product UI regression counts actual NOTE cards, not a presentation label ----
pt = Path('tests/paid_value_product_regression.cjs')
pts = pt.read_text(encoding='utf-8')
old_bundle = '''  modal = await page.locator('#unniProductModal').innerText();\n  assert((modal.match(/NOTE 0[1-6]/g) || []).length >= 18, 'bundle3 did not render three six-note reports');'''
new_bundle = '''  modal = await page.locator('#unniProductModal').innerText();\n  const bundleArticles = await page.locator('#unniProductBody article').count();\n  assert(bundleArticles === 18, `bundle3 NOTE card count ${bundleArticles}`);'''
if new_bundle not in pts:
    if old_bundle not in pts: raise SystemExit('bundle regression anchor not found')
    pts = pts.replace(old_bundle, new_bundle, 1)
pt.write_text(pts, encoding='utf-8')
