...
(async () => {
  ...
  assert(result.engine?.version === '6.1.1', 'NOTE v3 engine missing');
  ...
  for (const row of result.rows) {
    ...
    for (const [mode,notes,audit] of [['F',row.notesF,row.auditF],['T',row.notesT,row.auditT]]) {
      ...
      assert(audit?.version === '6.1.1' && audit?.engine === 'classical-causal-full-evidence', row.concern+'/'+row.situation+'/'+mode+': causal audit missing');
